...
def compute_cost(X, y, theta):
    ...
    scalar = 2 * len(X)
    matrixMultVector = X.dot(theta)

    vectorSub = np.subtract(matrixMultVector, y)
    vectorPowOf2 = vectorSub ** 2

    J = np.sum(vectorPowOf2) / scalar
...
def gradient_descent(X, y, theta, alpha, num_iters):
    ...
    for i in range(num_iters):
        ...
        vectorMultX = vectorSub.dot(X)

        J = vectorMultX * scalar
        theta = theta - J
...
def find_best_alpha(X, y, iterations):
    ...
    for i in range(len(alphas)):
        temp, alphaValue = efficient_gradient_descent(X, y, theta, alphas[i], iterations) 
        alpha_dict.update({alphas[i] : alphaValue[-1]})
...
def find_best_triplet(df, triplets, alpha, num_iter):
    ...
    for i in range(len(triplets)):
        columns_to_add = []
        for j in triplets[i]:
            columns_to_add.append(j)

        X = df[columns_to_add].values
        X, y = preprocess(X, y)
        X = np.c_[np.ones(len(X)), X]
        np.random.seed(42)
        theta = np.random.random(size = 4)

        theta, J_history = efficient_gradient_descent(X, y, theta, alpha, num_iter)
        best_current_cost.append(J_history[-1])
        
    index_best_triplet = best_current_cost.index(min(best_current_cost))
    best_triplet = list(triplets[index_best_triplet])
